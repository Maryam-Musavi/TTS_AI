...
with st.sidebar:
    ...
    # ⬇️ جدید: تنظیمات TTS در سایدبار
    st.subheader("🔊 تنظیمات صدا")
    
    tts_enabled = st.toggle(
        label="فعال‌سازی پخش صوتی پاسخ‌ها",
        value=True,
        help="با فعال‌سازی این گزینه، پاسخ‌ها به‌صورت صوتی پخش می‌شوند",
    )
    
    tts_voice = st.radio(
        label="انتخاب صدا",
        options=[constants.TTS_VOICE_FEMALE, constants.TTS_VOICE_MALE],
        format_func=lambda x: "زنانه" if "Dilara" in x else "مردانه",
        horizontal=True,
    )
...
if user_prompt:
    ...
    with st.chat_message(name=constants.AI):
        ...
        if not assistant_answer:
            ...
            # ⬇️ جدید: پخش صوتی پاسخ
            if tts_enabled and assistant_answer:
                try:
                    # اجرای async در محیط Streamlit
                    audio_file = asyncio.run(
                        functions.text_to_speech_async(
                    ...
                    if audio_file:
                        ...
                        # حذف فایل صوتی بعد از مدتی (اختیاری)
                        # import os
                        # os.remove(audio_file)
                        
                except Exception as e:
                    st.warning(f"⚠️ خطا در پخش صوتی: {e}")
