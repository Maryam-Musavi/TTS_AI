...
# ⬇️ جدید: ایمپورت ماژول TTS
from dt_tts_edge import convert_text_to_speech_async
import asyncio
...
async def text_to_speech_async(
    text: str,
    voice: str = constants.TTS_VOICE_FEMALE,
    rate: str = constants.TTS_RATE,
    pitch: str = constants.TTS_PITCH,
    volume: str = constants.TTS_VOLUME,
) -> Optional[str]:
    """
    ...
    # ۱. ایجاد پوشه temp اگر وجود ندارد
    if not os.path.exists(constants.TTS_TEMP_DIR):
        os.makedirs(constants.TTS_TEMP_DIR)
    
    # ۲. ساخت نام فایل یکتا بر اساس زمان
    timestamp: str = time.strftime("%Y%m%d_%H%M%S")
    audio_file_path: str = f"{constants.TTS_TEMP_DIR}/tts_{timestamp}.mp3"
    
    try:
        # ۳. تبدیل متن به گفتار
        word_count, elapsed_time = await convert_text_to_speech_async(
            text=text,
        ...
        st.error(f"❌ خطا در تبدیل متن به گفتار: {e}")
        return None